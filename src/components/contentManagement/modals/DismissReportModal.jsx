// src/components/contentManagement/modals/DismissReportModal.jsx
import React, { Fragment, useState } from "react";
import { Dialog, Transition } from "@headlessui/react";
import { TextField } from "@mui/material";
import { FileMinus2 } from "lucide-react";

function DismissReportModal({ open, onClose, report }) {
  const [note, setNote] = useState("");

  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Dismiss report", { report, note });
    onClose && onClose();
  };
                ...
                <form onSubmit={handleSubmit}>
                  <div className="flex items-center justify-between px-6 pt-5 pb-3">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-rose-50 flex items-center justify-center">
                        <FileMinus2 className="h-5 w-5 text-rose-500" />
                      </div>
                  ...
                  <div className="px-6 pb-6 space-y-6">
                    <p className="text-sm text-neutral-600">
                      You are about to mark this report as reviewed with no
                      action. The post will remain visible.
                    </p>
                    ...
                    <div className="space-y-2">
                      ...
                      <TextField
                        placeholder="Add a comment for internal record..."
                        multiline
                        minRows={4}
                        fullWidth
                        value={note}
